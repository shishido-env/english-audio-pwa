...
- [x] **Step 3: dev 起動して既存スタイル（Zinc + Violet）を確認**
...
- [x] **Step 1: ESLint 除外を一時的に外して lib / types / config だけスキャン**
...
- [x] **Step 2: 初期化を useState の lazy initializer に移す**
...
- [x] **Step 2: ref mutation を useEffect 内に移す**
...
- [x] **Step 3: `react-hooks/exhaustive-deps` 警告も解消**
...
- [x] **Step 1: components 配下に "use client" を付与**
...
- [x] **Step 2: shadcn/ui 配下にも "use client" を付与**
...
- [x] **Step 3: hooks 配下に "use client" を付与**
...
- [x] **Step 4: speechInstance に "use client" 付与**
...
- [x] **Step 1: 既存 src/App.tsx の最後のバージョンを git から取得**
...
- [x] **Step 1: globalIgnores から 'src' を外す**
...
- [x] `pnpm dev` で `http://localhost:3000` にライブラリ UI（既存文言）が HTTP 200 でレンダリング（手動の機能確認はユーザー側で実施）
- [x] `eslint.config.js` の `globalIgnores` から `'src'` が外れている（再有効化済み）
- [x] components.json が Next.js App Router 構成（`rsc: true`, `css: "app/globals.css"`）
- [x] 親 spec のフェーズ表で F2 が完了マーク

---

## 実装完了レポート（2026-05-28）

`feat/migrate-next` ブランチ上で F2 計 11 コミットを作成し、ローカル全チェック緑。

**作成コミット（古い順）:**

```
06fbd98 chore(f2): port theme tokens from src/index.css to app/globals.css
793f224 fix(f2): hoist useLibrary init to lazy useState initializer
49e80dd fix(f2): comply with react-hooks/refs and exhaustive-deps in usePlayer
d2b4c8b fix(f2): derive NowPlayingHero reveal state without effect
b7e44f3 fix(f2): satisfy react-hooks/refs in CsvFileInput
c83fc10 chore(f2): update components.json for next.js app router
bc01b75 chore(f2): mark client components/hooks with "use client"
0b944e9 feat(f2): port app shell into src/app-shell/LibraryShell.tsx
81aa942 feat(f2): wire LibraryShell from app/page.tsx
1aad95d feat(f2): pre-hydrate dark class to avoid theme FOUC
f9aaf1d chore(f2): re-enable eslint on src/ after F2 migration
```

**検証結果（クリーン再インストール後）:**
- `pnpm typecheck`: PASS（エラー 0）
- `pnpm lint`: PASS（エラー 0、警告は `next lint` deprecation 通知のみ）
- `pnpm test:run`: 7 files / 52 tests 全 PASS
- `pnpm build`: ✓ Compiled successfully、prerendered routes `/` と `/_not-found`
- `pnpm dev`: `http://localhost:3000` で HTTP 200、出力 HTML に既存 UI 文言（「デッキ」「読み上げ」「読み込み」「english」）を確認

**spec から逸脱した追加 fix:**
- **`src/hooks/useTheme.ts`, `src/hooks/useIntervals.ts`, `src/hooks/usePracticeMode.ts` の `loadXxx()` 関数に SSR ガード `if (typeof window === "undefined") return DEFAULT;` を追加**: Task 12 で `app/page.tsx` を `LibraryShell` に差し替えた際、`pnpm build` の prerender 段階で `ReferenceError: localStorage is not defined` が発生。`useLibrary` だけは plan 内で SSR ガードを書いていたが、他 3 hook も同じパターンが必要だった。Plan 同等の最小差分で対処（`81aa942` の同コミットに同梱）。

**F3 への申し送り事項:**
- `src/index.css` は `app/globals.css` への移植完了後も削除していない（F1 既存資産。`vite-plugin-pwa` 撤去後に参照されないので最終的に F3 か F4 で削除可）。
- 各 `loadXxx` の SSR ガードは「サインイン無し / localStorage ベース」現状の妥当な解。F6（DB 同期導入時）に Server Action からの初期データ流し込みが入る前提なら、それまでは現状方式で十分。
- `next lint` deprecation 警告は依然出ている（Next.js 16 で削除予定）。F3 着手前後で `package.json` の `lint` スクリプトを `eslint .` 直接呼び出しに切り替える検討推奨。
- `app/page.tsx` から `LibraryShell` を呼ぶ単一画面構成。F4 で Clerk 導入時に `(auth)` / `(app)` の route group へ再編する。
- 手動 UI 機能確認（CSV インポート、再生、デッキ切替、テーマ切替、暗記モード切替の golden path）はブラウザ実機テストが必要。本 plan の自動検証は HTML レンダリング有無まで。
